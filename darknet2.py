...
class Darknet(nn.Module):
    ...
    def load_weights(self, weightfile):
        ...
        header_version = np.fromfile(fp, count=3, dtype=np.int32)
        major, minor, revision = header_version
        self.header.append(header_version)
        if (major * 10 + minor >= 2)and major < 1000 and minor < 1000:
            self.seen = np.fromfile(fp, count=1, dtype=np.int64)
        else:
            self.seen = np.fromfile(fp, count=1, dtype=np.int32)
        self.header.append(self.seen)
        self.seen = self.seen[0]
        print('major, minor, revision, seen: ', major, minor, revision, self.seen)
    ...
    def save_weights(self, outfile, cutoff=0):
        ...
        fp = open(outfile, 'wb')
        self.header[1][0] = self.seen
        self.header[0].tofile(fp)
        self.header[1].tofile(fp)
